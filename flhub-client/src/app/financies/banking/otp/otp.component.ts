      ...
      for (let i = 0; i < $activityDates.length; i++) {
        ...
        if (currency === 'UAH') {
          ...
          for (let j = 0; j < $transactionsRows.length / rowsPerTransaction; j++) {
            ...
            const $amountCells = $firstRowCells.filter('.s18');
            const amount =
              parseFloat(($amountCells.eq(1).text() || '0').replace(' ', '')) -
              parseFloat(($amountCells.eq(0).text() || '0').replace(' ', ''));
            const transaction = {
              ...
              description: $firstRowCells.eq(9).text(),
          ...
          for (let j = 0; j < $transactionsRows.length / rowsPerTransaction; j++) {
            ...
            const usdAmount =
              ...
              parseFloat(($amountCells.eq(0).text() || '0').replace(' ', ''));
            const uahAmount =
              parseFloat(($amountCells.eq(3).text() || '0').replace(' ', '')) -
              parseFloat(($amountCells.eq(2).text() || '0').replace(' ', ''));
            const transaction = {
              date: $activityDate.text(),
              document: {
